...
from voice_modules.common.workflow_db import load_asr_rows as load_asr_view_rows
from voice_modules.common.workflow_db import read_workflow_rows, save_asr_view_rows


ASR_FIELDS = ["role", "file_name", "file_path", "model", "language", "transcript", "error"]
...
def recognize_once(audio_path: Path, model: str) -> tuple[str, str, dict[str, int]]:
...
def recognize_with_retry(audio_path: Path, model: str, max_attempts: int = 4, base_sleep: float = 2.0) -> tuple[str, str, dict[str, int]]:
    last_error = ""
    total_usage = {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0}
    for attempt in range(1, max_attempts + 1):
        text, error, usage = recognize_once(audio_path, model)
...
def run_asr(
    ...
    for row in workflow_rows:
        ...
        if should_run_row and (force or row.get("asr_status") != "done"):
    ...
    for index, preview_row in enumerate(pending_rows, start=1):
        ...
        transcript, error, usage = recognize_with_retry(audio_path, model)
        add_usage(usage_totals, usage)
        preview_row["model"] = model
        preview_row["language"] = "zh"
        preview_row["transcript"] = transcript
        preview_row["error"] = error
        if index % 5 == 0 or index == total:
            print(f"[ASR] 已处理 {index}/{total} 条。", flush=True)
    error_count = sum(1 for row in pending_rows if row.get("error"))
    print(f"[ASR] 运行完成。成功 {len(pending_rows) - error_count} 条，失败 {error_count} 条。", flush=True)
    if error_count:
        failed_items = [row.get("file_name", "") for row in pending_rows if row.get("error")]
    ...
    print("[ASR] 本次结果仅供预览，点击“保存 ASR”后才会正式写入 workflow_db.csv。", flush=True)
    return preview_rows
