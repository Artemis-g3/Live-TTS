...
def _has_garbled_text(value: object) -> bool:
    return any("\ue000" <= char <= "\uf8ff" for char in str(value))
...
def _load_excel_rows(excel_path: Path) -> list[dict[str, str]]:
    ...
    if missing:
        garbled = sorted({str(column) for column in df.columns if _has_garbled_text(column)})
        detail = f"（检测到表头含乱码字符的列: {garbled}，请检查 Excel 表头编码）" if garbled else ""
        raise ImportValidationError(f"Excel 缺少必要列: {missing}{detail}")
...
def _write_workflow_db(role: str, excel_rows: list[dict[str, str]]) -> tuple[Path, list[dict[str, str]]]:
    ...
    for row in workflow_rows:
        ...
        row["asr_language"] = "zh"
        row["asr_original_text"] = _clean_text(excel_row["语音文本"])
        row["asr_transcript"] = _clean_text(excel_row["语音文本"])
        row["asr_error"] = ""
        row["asr_updated_at"] = timestamp
        row["translation_status"] = "done"
        row["translation_model"] = "direct_copy"
        row["translation_error"] = ""
        row["translation_updated_at"] = timestamp
