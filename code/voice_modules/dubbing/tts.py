...
DEFAULT_SYNTHESIS_LANGUAGE = "zh"
SYNTHESIS_LANGUAGE_CONFIG = {
    "zh": {"label": "中文", "tts_language_type": "Chinese"},
    "en": {"label": "英语", "tts_language_type": "English"},
    "ja": {"label": "日语", "tts_language_type": "Japanese"},
}
...
def resolve_synthesis_language(language: str) -> dict[str, str]:
    key = str(language or "").strip().lower() or DEFAULT_SYNTHESIS_LANGUAGE
    config = SYNTHESIS_LANGUAGE_CONFIG.get(key)
    if config is None:
        supported = ", ".join(sorted(SYNTHESIS_LANGUAGE_CONFIG))
        raise ValueError(f"不支持的合成语言: {language}。支持: {supported}")
...
def _concat_reference_audio(
    ...
    *,
    ...
    session_dir: Path,
    selected_results: list[dict[str, Any]] | None,
    reference_text_key: str,
    audio_stem: str,
    text_stem: str,
) -> dict[str, Any]:
    if not top_results and not selected_results:
    ...
    for row in source_rows:
        ...
        selected.append(row)
        reference_text = str(row.get(reference_text_key, "") or "").strip() or str(row.get(COL_TRANSCRIPT, "") or "").strip()
        texts.append(reference_text)
        source_languages.append(str(row.get("asr_language", "") or "").strip())
    ...
    audio_path_out = run_file_path(session_dir, audio_stem, ".wav")
    sf.write(str(audio_path_out), wav, sample_rate, subtype="PCM_16")
    reference_text = "\n".join(texts)
    text_path_out = run_file_path(session_dir, text_stem, ".txt")
    text_path_out.write_text(reference_text, encoding="utf-8")
    return {
        "selected_results": selected,
        "audio_path": audio_path_out,
        "text_path": text_path_out,
        "text": reference_text,
        "sample_rate": sample_rate,
        "duration_seconds": len(wav) / sample_rate,
        "source_languages": [item for item in source_languages if item],
    }
...
def build_reference_audio(
    ...
    *,
    ...
    reference_text_key: str = COL_TRANSCRIPT,
    style_results: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    timbre = _concat_reference_audio(
        top_results,
        ...
        audio_stem="reference_audio",
        text_stem="reference_text",
    )
    result: dict[str, Any] = {
        "selected_results": timbre["selected_results"],
        "reference_audio_path": timbre["audio_path"],
        "reference_text_path": timbre["text_path"],
        "reference_text": timbre["text"],
        "sample_rate": timbre["sample_rate"],
        "duration_seconds": timbre["duration_seconds"],
        "source_languages": timbre["source_languages"],
    }
    if style_results is not None:
        style_text_key = "asr_original_text"
        style = _concat_reference_audio(
            top_results,
            reference_window=reference_window,
            gap_ms=gap_ms,
            reference_min_seconds=reference_min_seconds,
            session_dir=session_dir,
            selected_results=style_results,
            reference_text_key=style_text_key,
            audio_stem="style_audio",
            text_stem="style_text",
        )
        result["style_selected_results"] = style["selected_results"]
        result["style_audio_path"] = style["audio_path"]
        result["style_text_path"] = style["text_path"]
        result["style_text"] = style["text"]
        result["style_sample_rate"] = style["sample_rate"]
        result["style_duration_seconds"] = style["duration_seconds"]
        result["style_source_languages"] = style["source_languages"]
    return result
...
def synthesize_api_tts(text: str, voice: str, target_model: str, language_type: str) -> tuple[str, Any]:
...
def compute_local_prompt_fingerprint(reference_audio_path: Path, reference_text: str, model_path: Path, mode: str, style_audio_path: Path | None = None, style_text: str = "") -> str:
    payload = hashlib.sha256()
    payload.update(reference_audio_path.read_bytes())
    payload.update(reference_text.encode("utf-8"))
    payload.update(str(model_path.resolve()).encode("utf-8"))
    payload.update(mode.encode("utf-8"))
    if style_audio_path is not None:
        payload.update(style_audio_path.read_bytes())
        payload.update(style_text.encode("utf-8"))
    return payload.hexdigest()


def create_or_load_prompt_cache(model: VoxCPM, reference_audio_path: Path, reference_text: str, mode: str, cache_index_path: Path, prompts_dir: Path, model_path: Path, style_audio_path: Path | None = None, style_text: str = "") -> tuple[dict[str, Any], str, bool]:
    fingerprint = compute_local_prompt_fingerprint(reference_audio_path, reference_text, model_path, mode, style_audio_path, style_text)
    ...
    if mode == "basic_clone":
        ...
        prompt_wav = str(style_audio_path) if style_audio_path is not None else str(reference_audio_path)
        prompt_text = style_text if style_text else reference_text
...
def synthesize_voxcpm2_tts(
    ...
    style_audio_path: Path | None = None,
    style_text: str = "",
) -> tuple[Path, str, bool]:
    model = load_local_model(model_path)
    prompt_cache, fingerprint, used_cache = create_or_load_prompt_cache(
        model, reference_audio_path, reference_text, mode, cache_index_path, prompts_dir, model_path,
        style_audio_path=style_audio_path, style_text=style_text,
    )
...
def run_synthesis_from_retrieval(
    *,
    ...
    session_dir: Path,
    synthesis_language: str = DEFAULT_SYNTHESIS_LANGUAGE,
    selected_results: list[dict[str, Any]] | None = None,
    style_results: list[dict[str, Any]] | None = None,
    ...
    language_config = resolve_synthesis_language(synthesis_language)
    if not transcript_text.strip():
        raise ValueError("配音台词不能为空。")
    if not retrieval_voice_guidance.strip():
        raise ValueError("检索声音指导不能为空。")
    ...
    print(
        f"[TTS] 合成语言={language_config['label']} "
        f"(key={language_config['key']}, api={language_config['tts_language_type']})",
        flush=True,
    )
    print(f"[TTS] 参考音频语言={reference_language}", flush=True)
    reference_text_source = "original_text" if backend == "voxcpm2_local_hifi" else "chinese_text"
    reference_text_key = "asr_original_text" if reference_text_source == "original_text" else COL_TRANSCRIPT
    ...
    if backend == "api":
        ...
        audio_url, tts_response = synthesize_api_tts(
            ...
            voice,
            target_model,
            language_config["tts_language_type"],
        )
        ...
        output_audio_path, voxcpm2_prompt_fingerprint, used_cached_voxcpm2_prompt = synthesize_voxcpm2_tts(
            ...
            style_audio_path=reference_bundle.get("style_audio_path"),
            style_text=reference_bundle.get("style_text", ""),
        )
        api_fingerprint = ""
    ...
    run_summary = {
        "role": role_paths.role,
        "backend": backend,
        "synthesis_language": language_config["key"],
        "synthesis_language_label": language_config["label"],
        "reference_language": reference_language,
        "tts_language_type": language_config["tts_language_type"],
        ...
        "reference_text_language": (
            ",".join(sorted(set(reference_bundle.get("source_languages", []))))
            if reference_text_source == "original_text"
            else "zh"
        ),
        ...
        "style_audio_path": str(reference_bundle["style_audio_path"]) if reference_bundle.get("style_audio_path") is not None else "",
        "style_selected_results": reference_bundle.get("style_selected_results", []),
    }
