...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.asr_hidden_headers = {"sha256", "role", "file_path", "model", "language"}
    ...
    def _build_asr_tab(self) -> None:
        ...
        run_asr_btn = QPushButton("运行 ASR")
        run_asr_btn.clicked.connect(self.run_asr)
        run_single_asr_btn = QPushButton("运行单条 ASR")
        ...
        save_asr_btn = QPushButton("保存 ASR")
        ...
        for widget in [run_asr_btn, run_single_asr_btn, stop_asr_btn, load_asr_btn, save_asr_btn, clear_asr_log_btn]:
    ...
    def _build_emotion_tab(self) -> None:
        ...
        run_emotion_btn = QPushButton("运行情感标定")
        run_emotion_btn.clicked.connect(self.run_emotion)
        run_single_emotion_btn = QPushButton("运行单条情感标定")
        run_single_emotion_btn.clicked.connect(self.run_single_emotion)
        ...
        for widget in [run_emotion_btn, run_single_emotion_btn, stop_emotion_btn, load_emotion_btn, save_emotion_btn, clear_emotion_log_btn]:
    ...
    def _build_dubbing_tab(self) -> None:
        ...
        self.reference_window_spin = QSpinBox()
        self.reference_window_spin.setRange(1, 50)
        self.reference_window_spin.setValue(self.config.reference_window)
        self.reference_min_spin = QDoubleSpinBox()
        self.reference_min_spin.setRange(1.0, 120.0)
        self.reference_min_spin.setDecimals(1)
        self.reference_min_spin.setValue(self.config.reference_min_seconds)
        ...
        form.addRow("检索声音指导", self.retrieval_guidance_edit)
        form.addRow("合成声音指导", self.synthesis_guidance_edit)
        form.addRow("第二轮候选音频数量", self.reference_window_spin)
        form.addRow("参考音频最小时长", self.reference_min_spin)
        form.addRow("语音合成扩散轮数", self.voxcpm2_diffusion_steps_spin)
        form.addRow("VoxCPM2 cfg_value", self.voxcpm2_cfg_spin)
        buttons = QHBoxLayout()
    ...
    def _build_settings_tab(self) -> None:
        ...
        grid.addWidget(QLabel("Bai Lian API Key"), 2, 0)
        ...
        model_rows = [
            ...
            ("情感标定模型", self.emotion_model_edit),
            ("检索 rerank 模型", self.retrieval_rerank_model_edit),
            ("最终排序文本模型", self.retrieval_text_model_edit),
    ...
    def _build_output_overview_tab(self) -> None:
        ...
        refresh_btn.clicked.connect(self.load_output_overview)
    ...
    def save_settings_without_dialog(self) -> None:
        ...
        self.config.reference_window = self.reference_window_spin.value()
        self.config.reference_min_seconds = self.reference_min_spin.value()
    ...
    def run_emotion(self) -> None:
        ...
        if not self.emotion_model_edit.currentText().strip():
            ...
            return
        args = [
            "-m", "GUI.backend.workflow_cli", "run-emotion",
            "--role", role,
            "--api-key", self.api_key_edit.text().strip(),
    ...
    def run_single_emotion(self) -> None:
        ...
        sha256 = str(row.get("sha256", "") or "").strip()
        if not sha256:
            QMessageBox.warning(self, "情感标定", "所选音频缺少 sha256，无法运行单条情感标定。")
            return
        if not str(row.get("语音文本", "") or "").strip():
            QMessageBox.warning(self, "情感标定", "该音频尚未完成 ASR，无法运行情感标定。")
            return
        role = self.emotion_role_combo.currentText()
        if not self.emotion_model_edit.currentText().strip():
            QMessageBox.warning(self, "情感标定", "情感标定模型不能为空。")
    ...
    def load_output_overview(self) -> None:
        ...
        if not role:
            ...
            return
        self.start_process(["-m", "GUI.backend.workflow_cli", "list-tts-runs", "--role", role], self.handle_output_overview_result)
    ...
    def synthesize_tts(self) -> None:
        ...
        if not self.last_session_dir:
            ...
            return
        selected_rows = self.selected_retrieval_rows()
        if not selected_rows:
            QMessageBox.warning(self, "配音", "请至少勾选一条参考音频。")
            return
        selected_duration_seconds = self.selected_reference_duration_seconds(selected_rows)
        minimum_seconds = float(self.reference_min_spin.value())
        if selected_duration_seconds < minimum_seconds:
            QMessageBox.warning(
                self,
                "参考音频时长不足",
                f"当前勾选参考音频总时长为 {selected_duration_seconds:.1f} 秒，小于设定的参考音频最小时长 {minimum_seconds:.1f} 秒。",
            )
            return
        selected_results_path = self.write_runtime_json("retrieval_selection.json", selected_rows)
        args = [
            ...
            "--reference-window", str(self.reference_window_spin.value()),
            "--reference-min-seconds", str(self.reference_min_spin.value()),
    ...
    def validate_tts_inputs(self, *, require_backend: bool) -> bool:
        ...
        if not self.api_key_edit.text().strip():
            QMessageBox.warning(self, "输入检查", "检索流程需要 Bai Lian API Key，请先在设置页填写。")
            return False
    ...
    def handle_retrieval_result(self, result: dict[str, Any]) -> None:
        ...
        self.apply_default_retrieval_selection()
    ...
    def populate_retrieval_table(self) -> None:
        headers = ["用作参考", "语音文件", "音频时长(秒)", "语音文本", "情绪语气", "语音技巧"]
        ...
        for r, row in enumerate(self.retrieval_rows):
            ...
            checkbox = QCheckBox()
            checkbox.setChecked(bool(row.get("selected_for_reference", False)))
            checkbox_container = QWidget()
            checkbox_layout = QHBoxLayout(checkbox_container)
            checkbox_layout.setContentsMargins(0, 0, 0, 0)
            checkbox_layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
            checkbox_layout.addWidget(checkbox)
            self.retrieval_table.setItem(r, 0, checkbox_item)
            self.retrieval_table.setCellWidget(r, 0, checkbox_container)
    ...
    def apply_default_retrieval_selection(self) -> None:
        total_seconds = 0.0
        selected_count = 0
        gap_seconds = float(self.gap_spin.value()) / 1000.0
        minimum_seconds = float(self.reference_min_spin.value())
        max_candidates = int(self.reference_window_spin.value())
        for row in self.retrieval_rows:
            row["selected_for_reference"] = False
        for row in self.retrieval_rows[:max_candidates]:
            duration_seconds = float(row.get("duration_seconds", 0) or 0)
            if duration_seconds > minimum_seconds:
                continue
            if selected_count > 0:
                total_seconds += gap_seconds
            row["selected_for_reference"] = True
            total_seconds += duration_seconds
            selected_count += 1
            if total_seconds > minimum_seconds:
                break
        if selected_count == 0 and self.retrieval_rows:
            self.retrieval_rows[0]["selected_for_reference"] = True

    def selected_retrieval_rows(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for row_index, base_row in enumerate(self.retrieval_rows):
            item = self.retrieval_table.item(row_index, 0)
            widget = self.retrieval_table.cellWidget(row_index, 0)
            checkbox = widget.findChild(QCheckBox) if widget is not None else None
            selected = bool(base_row.get("selected_for_reference", False))
            if checkbox is not None:
                selected = checkbox.isChecked()
            elif item is not None:
                selected = item.checkState() == Qt.CheckState.Checked
            base_row["selected_for_reference"] = selected
            if selected:
    ...
    def asr_headers(self) -> list[str]:
        return ["sha256", "role", "file_name", "file_path", "model", "language", "transcript", "error"]
