...
STYLE_AUDIO_PREFIX = "style_audio"
STYLE_TEXT_PREFIX = "style_text"
...
def _is_conforming_run_dir(run_dir: Path) -> bool:
    ...
    if len(synthesized_files) != 1 or len(reference_audio_files) != 1 or len(reference_text_files) != 1:
        return False
    style_audio_files = _match_prefixed_files(run_dir, STYLE_AUDIO_PREFIX, {".wav"})
    style_text_files = _match_prefixed_files(run_dir, STYLE_TEXT_PREFIX, {".txt"})
    if len(style_audio_files) > 1 or len(style_text_files) > 1:
        return False
    allowed = (
        REQUIRED_FIXED_FILENAMES
        | {path.name for path in synthesized_files}
        | {path.name for path in reference_audio_files}
        | {path.name for path in reference_text_files}
        | {path.name for path in style_audio_files}
        | {path.name for path in style_text_files}
    )
    for path in files:
